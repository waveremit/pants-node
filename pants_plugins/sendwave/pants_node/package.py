...
async def get_node_search_paths(
    ...
    search_paths = node.options.search_paths
    if node.options.use_nvm:
    ...
    if not npm_paths.first_path:
        raise ValueError(
            "Could not find npm in: ({}) cannot create package".format(
                search_paths
            )
        )
    return NPMPath(
        binary_path=npm_paths.first_path.path, search_paths=search_paths
    )
...
async def get_node_package_digest(field_set: NodeProjectFieldSet) -> Digest:
    ...
    source_files = await Get(
        SourceFiles, NodeSourceFilesRequest(field_set.address)
    )
    ...
    build_context = await Get(
        Digest,
        MergeDigests([stripped_files.digest, npm_install_result.output_digest]),
    )
    ...
    if field_set.output_path and field_set.output_path.value is not None:
        return await Get(
            Digest,
            AddPrefix(build_result.output_digest, field_set.output_path.value),
        )
    else:
        output = await Get(
            Digest, AddPrefix(build_result.output_digest, package_root)
        )
